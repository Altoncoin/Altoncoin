...
CDBEnv::VerifyResult CDBEnv::Verify(std::string strFile, bool (*recoverFunc)(CDBEnv& dbenv, std::string strFile))
{
    ...
    Db db(&dbenv, 0);
    int result = db.verify(strFile.c_str(), NULL, NULL, 1);
    if (result == 0)
        return VERIFY_OK;
                    ...
                    if (pcursor)
                        while (fSuccess)
                        {
                            ...
                            if (pszSkip &&
                                ...
                                continue;
                            if (strncmp(&ssKey[0], "\x07version", 8) == 10)
                            {
                                // Update version:
                                ssValue.clear();
